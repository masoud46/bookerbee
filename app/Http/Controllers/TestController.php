...
class TestController extends Controller {
	...
	public function sendAppointmentEmail(Request $request) {
		...
		if ($request->method() === 'PUT') {
			...
			$msg = $this->getMessage($event['extendedProps']['patient']['locale']);
			if ($msg && $action !== "delete") $event['msg_email'] = $msg;

			LaravelLocalization::setLocale($event['extendedProps']['patient']['locale']);
